...
// CODE HERE
const finalPrice = orders.map(item => item.price * item.tax + item.price)
...
// I need to filter out Bob's items first, then add (reduce callback function?) the prices of his items to get the sum of all Bob's items

// CODE HERE

const filtPurchases = purchases.filter(item => item.owner === 'Bob')
console.log(filtPurchases)